...
| [`disclosure`](#acquaint.disclosure)(people, \*[, projects, audience, ...])  | Who may be told what, before writing to a set of readers (ids or channel identities, plus an optional correspond `audience` record as JSON): the tier and clearance in force for each, the least clearance, which records each is cleared for, the seals, the vocabulary a gate must scan for, what each was already told, and the gaps. |
...
| [`deslop`](acquaint.deslop.md#module-acquaint.deslop)       | The deterministic half of deslop: find the patterns that make prose read as machine-written, scaled to the reader.                                                               |
|--------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`edit`](acquaint.edit.md#module-acquaint.edit)           | Changing the store: create an entity, append an observation, rename with relinking, forget with a tombstone.                                                                     |
| [`lookup`](acquaint.lookup.md#module-acquaint.lookup)       | Finding the right record: exact references, handle resolution, the conflation check, and channel choice.                                                                         |
| [`mcp`](acquaint.mcp.md#module-acquaint.mcp)             | MCP over stdio: the same tools, for Claude Desktop and other local MCP clients.                                                                                                  |
| [`records`](acquaint.records.md#module-acquaint.records)     | The text formats acquaint stores, as pure functions: frontmatter, sections, items, source tags, slugs, log entries.                                                              |
| [`render`](acquaint.render.md#module-acquaint.render)       | Turning a tool's result into terminal output: `(stdout, stderr, exit code)`.                                                                                                     |
| [`resources`](acquaint.resources.md#module-acquaint.resources) | Files shipped inside the package: record templates, the policy tripwires, purpose reminders, the tells catalogue, skills.                                                        |
| [`store`](acquaint.store.md#module-acquaint.store)         | Where records live and how code reaches them: the data root, [`Store`](#acquaint.Store) and [`Entity`](#acquaint.Entity). |
| [`sync`](acquaint.sync.md#module-acquaint.sync)           | Private-repository sync for the data root: create the repository private, refuse anything else, guard every push.                                                                |
| [`tools`](acquaint.tools.md#module-acquaint.tools)         | The single source of truth for every surface: plain functions, flat arguments in, JSON-ready dicts out.                                                                          |
| [`trust`](acquaint.trust.md#module-acquaint.trust)         | The disclosure vocabulary: tiers on people; labels, seals and clearances on records; and which tier is in force.                                                                 |
