...
def run_command(
    args: Sequence[str], *, cwd: str | os.PathLike | None = None
    ...
    try:
        ...
        raise AcquaintError(
            f"{args[0]} is not installed or not on PATH ({error})"
        ) from error
...
def _ok(run: Runner, args: Sequence[str], *, cwd=None, what: str) -> str:
    ...
    if result.returncode != 0:
        ...
        raise AcquaintError(
            f"{what} failed: {' '.join(args)}: {detail[-1] if detail else 'no output'}"
        )
...
def _lines(root: Path, run: Runner, *args: str) -> list[str]:
    ...
    return [
        line
        for line in (run(["git", *args], cwd=root).stdout or "").splitlines()
        if line.strip()
    ]
...
def _require_private(repo: str, run: Runner) -> str:
    ...
    if seen != "PRIVATE":
        raise AcquaintError(
            f"{repo} is {seen or 'of unknown visibility'}, not PRIVATE: refusing to put profile data in it"
        )
    return seen
...
def _configured(root: Path, run: Runner) -> tuple[str, str]:
    if not (root / ".git").exists():
        raise AcquaintError(
            f"{root} is not a synced store yet: run `acquaint sync init --repo <owner>/<name>` first"
        )
    ...
    if not repo or not remote:
        raise AcquaintError(
            f"{root} is a git repository but was not set up by `acquaint sync init`"
        )
    if not _same_repo(remote, repo):
        raise AcquaintError(
            f"the recorded remote {remote!r} is not a url of the GitHub repository {repo}; refusing"
        )
    urls = _lines(root, run, "config", "--get-all", "remote.origin.url")
    if urls != [remote]:
        raise AcquaintError(
            f"origin must have exactly one url, {remote!r}, but has {urls}; refusing"
        )
    pushurls = _lines(root, run, "config", "--get-all", "remote.origin.pushurl")
    if pushurls not in ([], [remote]):
        raise AcquaintError(
            f"origin has a pushurl {pushurls} that differs from {remote!r}; refusing"
        )
    fetching = _lines(root, run, "remote", "get-url", "--all", "origin")
    pushing = _lines(root, run, "remote", "get-url", "--push", "--all", "origin")
    if pushing != fetching:
        raise AcquaintError(
            f"git would push origin to {pushing} but fetch from {fetching} (a url.*.pushInsteadOf rewrite?); refusing"
        )
    return repo, remote
...
def _hook_active(root: Path, run: Runner) -> bool:
    ...
    if not hook.is_file() or hook.read_text(encoding="utf-8") != data_text(
        "hooks/pre-push"
    ):
        return False
...
def _install_hook(root: Path, run: Runner) -> Path:
    ...
    _ok(
        ...
        cwd=root,
        what="pointing git at the pre-push guard",
    )
    return hook
...
def _check_existing_checkout(root: Path, repo: str, run: Runner) -> None:
    ...
    if recorded:
        if recorded.lower() != repo.lower():
            raise AcquaintError(
                f"{root} already syncs to {recorded}; refusing to point it at {repo}"
            )
        return
...
def init(
    ...
    *,
    ...
    plan = [
        f"gh repo view {repo} (create it with `gh repo create {repo} --private` if missing)"
        if create
        else f"gh repo view {repo}",
    ...
    if not exists:
        if not create:
            raise AcquaintError(
                f"{repo} does not exist (drop --existing-only to create it, private)"
            )
        _ok(
            run,
            [
                ...
                "--private",
                "--description",
                "acquaint profile store (private)",
            ],
            what="creating the repository",
        )
    seen = _require_private(repo, run)
    url = remote_url or _ok(
        run,
        ["gh", "repo", "view", repo, "--json", "sshUrl", "-q", ".sshUrl"],
        what="reading the SSH URL",
    )
    ...
    _ok(
        run,
        ["git", "config", "acquaint.repo", repo],
        cwd=root,
        what="recording the repository",
    )
    _ok(
        run,
        ["git", "config", "acquaint.remote", url],
        cwd=root,
        what="recording the remote",
    )
    ...
    try:
        ...
        raise AcquaintError(
            f"the store is set up for {repo}, but the first push failed: {error}. Fix that, then run `acquaint sync push`"
        ) from error
    return {
        "repo": repo,
        "root": str(root),
        ...
        "visibility": seen,
        "hook": str(hook),
        **{k: pushed[k] for k in ("committed", "pushed")},
    }
...
def push(
    ...
    *,
    ...
    changed = [
        line
        for line in _ok(
            run, ["git", "status", "--porcelain"], cwd=root, what="git status"
        ).splitlines()
        if line
    ]
    if dry_run:
    ...
    if changed:
        ...
        _ok(
            ...
            [
                ...
                "-q",
                "-m",
                message or f"acquaint: sync {stamp} ({len(changed)} files)",
            ],
            cwd=root,
            what="git commit",
        )
    ...
    if run(["git", "fetch", "-q", "origin", BRANCH], cwd=root).returncode == 0:
        _ok(
            ...
            cwd=root,
            what="rebasing onto the remote",
        )
    _ok(
        run,
        ["git", "push", "-q", "-u", "origin", f"HEAD:{BRANCH}"],
        cwd=root,
        what="git push (the pre-push guard may have refused)",
    )
    return {
        "repo": repo,
        "remote": remote,
...
def pull(
    root: str | os.PathLike, *, dry_run: bool = False, run: Runner = run_command
) -> dict[str, Any]:
    ...
    if dry_run:
        return {
            "repo": repo,
            "dry_run": True,
            "plan": [f"git pull --rebase --autostash origin {BRANCH}"],
        }
    _require_private(repo, run)
    _ok(
        run,
    ...
    head = _ok(
        run, ["git", "rev-parse", "--short", "HEAD"], cwd=root, what="git rev-parse"
    )
...
def status(
    ...
    if not (root / ".git").exists():
        return {
            "synced": False,
            "root": str(root),
            "note": "not set up: acquaint sync init --repo <owner>/<name>",
        }
    repo, remote = _configured(root, run)
    dirty = [
        line
    ...
    if check_visibility:
        ...
        if run(["git", "fetch", "-q", "origin", BRANCH], cwd=root).returncode == 0:
            counts = _ok(
                ...
                cwd=root,
                what="comparing with the remote",
            )
